...
<div id="contenido">
<div id="navegacion2"><a href="principal.php">inicio</a> > convocatorias activas</div>
<form name="consultaconcursosactivos" method="post" action="concurso_activo.php">
<div id="buscador">
...
<?php
	...
	while(is_array($res) && (list($Id,$Dato) = each($res)) && ($cont < $ultimo - 1)){
		...
		$Descripcion = substr(quitar_caracteres_filtros($Dato[Descripcion]),0,50)."...";
...
?>
	...
	<h1><a href="detalles_concurso_activo.php?IdConcurso=<?php echo $Dato[IdConcurso]; ?>" target="_top" title="ver detalles concurso"><?php echo $NombreConcurso; ?></a></h1>
...
<div class="parrafoprincipal4">
           ...
           <!--Comentarios -->
				<?php 
					//if (is_array($res_com) && (list($Id,$Dato1)=each($res_com))){
					if (is_array($res_com)){ 
					?>
			...
			<?php echo $proyectos_concurso; ?><br />
...
<?php 
		...
		if ($primero > 0){
			...
			echo "<a href=\"concurso_activo.php?cadena=$cadena&Modalidad=$Modalidad&Disciplina=$Disciplina&primero=$anterior&ban=get\" title=\"anterior\"><img src=\"iconos/left.gif\"/></a>";
		}else{echo "&nbsp;";}           
?>
...
<?php 
		if ($total_items > $ITEMS_CONSULTAS){
			...
			echo "<a href=\"concurso_activo.php?cadena=$cadena&Modalidad=$Modalidad&Disciplina=$Disciplina&primero=$siguiente&ban=get\" title=\"siguiente\"><img src=\"iconos/right.gif\"/></a>";
		}else{echo "&nbsp;";}           
?>			  
</div></td>
            </tr>
